...
class amaq(object):
    ...
    def meaning_check(self, text: str) -> List[str]:
        ...
        if text.correct() != text:
            while True:
                ...
                if choice.lower() not in ("y", "n"):
                    rich_print(
                        "[yellow]Valid options are:[/yellow] [bold red]Y[/bold red] and [bold red]N[/bold red] only."
                    )
                    continue
    ...
    def quit(self) -> None:
        ...
        logging.info(
            f"Terminating script at {time.asctime(time.localtime(time.time()))}"
        )
        exit()
